...
// the amount is locked at creation, the destination is allowlisted by the
// service, and a replayed idempotency_key returns the same intent instead of
// double-spending. The private key never enters the DB — only the resulting
...
const WalletSpend = model.define("wallet_spend", {
  ...
  // Destination wallet address the funds moved to (allowlisted by the service:
  // a platform treasury / payment-session deposit address).
