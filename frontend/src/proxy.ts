...
async function getRegionMap(cacheId: string) {
  ...
  try {
    ...
    if (!regions?.length) {
      throw new Error(
        "No regions found. Please set up regions in your How's u backend."
      )
    }
...
export async function proxy(request: NextRequest) {
  ...
  if (!urlHasCountryCode && countryCode) {
    ...
    return new NextResponse(
      "No valid regions configured. Please set up regions with countries in your How's u backend.",
      { status: 500 }
    )
  }
...
export const config = {
  matcher: [
    "/((?!api|warm|_next/static|_next/image|favicon.ico|images|assets|png|svg|jpg|jpeg|gif|webp).*)",
  ],
}
