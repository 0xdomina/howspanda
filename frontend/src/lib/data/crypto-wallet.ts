...
// SECURITY: all wallet operations go through the backend store routes ONLY.
// The backend owns the Circle credentials, the password gate on withdrawals,
// the spend ledger and the reconcile sweeps — a storefront-side direct Circle
// call would bypass every one of those controls. Never re-introduce a
// frontend Circle client (see docs: "one settlement owner — the backend").
