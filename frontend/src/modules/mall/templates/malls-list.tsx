...
const WinTicker = ({ wins }: { wins: MallWin[] }) =>
  wins.length ? (
    <div className="mb-8 rounded-control border border-ink-hairline bg-white px-4 py-3">
      <p className="text-xs font-medium uppercase tracking-wide text-ink-muted">Recent wins</p>
      <ul className="mt-2 flex flex-col gap-1.5">
        {wins.map((win) => (
          <li key={win.id} className="flex items-center justify-between gap-3 text-sm">
            <span className="truncate text-ink-muted">
              <span className="text-ink">{maskEmail(win.winner_buyer_email)}</span> won in{" "}
              <LocalizedClientLink href={`/malls/${win.mall_id}`} className="hover:underline">
                {win.mall_name}
              </LocalizedClientLink>
            </span>
            <span className="shrink-0 font-mono tabular-nums text-emerald-700">+{ngn(win.amount_ngn)}</span>
          </li>
        ))}
      </ul>
    </div>
        ...
        <div className="min-w-0">
          <div className="flex flex-wrap items-center gap-2">
            <LocalizedClientLink href={`/malls/${mall.id}`}>
              <h2 className="font-display text-lg font-medium text-ink hover:underline">{mall.name}</h2>
            </LocalizedClientLink>
            <span className="text-xs text-ink-muted">Rank #{rank}</span>
            {rank === 1 && <span className="rounded-full bg-amber-50 px-2 py-0.5 text-xs font-medium text-amber-800">Top mall</span>}
          </div>
          <p className="mt-1 line-clamp-2 text-sm text-ink-muted">{mall.description || "A community shopping event."}</p>
        </div>
        <span className={`shrink-0 rounded-full px-2 py-0.5 text-xs ${pending ? "bg-amber-50 text-amber-800" : "bg-emerald-50 text-emerald-700"}`}>
          {pending ? "Pending launch" : "Shopping open"}
        </span>
      </div>

      <div className="mt-4 grid grid-cols-3 gap-2 text-center">
        <div className="rounded-medium bg-ink/5 px-2 py-2"><p className="text-xs text-ink-muted">Pool</p><p className="mt-0.5 font-mono text-sm text-ink">{ngn(mall.prize_pool_ngn)}</p></div>
        <div className="rounded-medium bg-ink/5 px-2 py-2"><p className="text-xs text-ink-muted">Paid</p><p className="mt-0.5 font-mono text-sm text-ink">{ngn(mall.paid_out_ngn)}</p></div>
        <div className="rounded-medium bg-ink/5 px-2 py-2"><p className="text-xs text-ink-muted">Community</p><p className="mt-0.5 font-mono text-sm text-ink">{mall.seller_count ?? 0}/{mall.target_sellers} sellers</p></div>
      </div>
    ...
    <div data-testid="malls-page" className="figma-container flex-1 py-10 small:py-16">
      <div className="mb-8 flex flex-wrap items-end justify-between gap-4">
        <div className="max-w-2xl">
          <h1 className="font-display text-3xl font-medium tracking-[-0.02em] text-ink">Malls</h1>
          <p className="mt-2 max-w-xl text-sm text-ink-muted">Community shopping events. Join early, browse featured products, and shop when a mall reaches its launch target.</p>
        </div>
        {hasSellerTools ? (
          <LocalizedClientLink href="/malls/create" className="rounded-control bg-ink px-4 py-3 text-sm font-medium text-white">Create a mall</LocalizedClientLink>
        ) : customerEmail ? (
          <LocalizedClientLink href="/seller" className="rounded-control border border-ink-strong px-4 py-3 text-sm font-medium text-ink">Become a seller</LocalizedClientLink>
        ) : (
          <LocalizedClientLink href="/account" className="rounded-control border border-ink-strong px-4 py-3 text-sm font-medium text-ink">Sign in to create a mall</LocalizedClientLink>
        )}
      </div>
